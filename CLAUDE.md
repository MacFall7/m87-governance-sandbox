...
M87 Governance Sandbox — a pure-function state machine proving 7 architectural invariants hold under 23 adversarial injection scenarios.

## Commands

```bash
npm test          # Run all tests (must be 24/24)
...
- `src/core/injections.ts` — 23 fault injection definitions (frozen)
- `src/core/failureMatrix.ts` — Maps injections to expected triggers/invariants (frozen)
- `test/governanceReducer.test.ts` — 23 failure matrix tests + 1 structural test
